...
class NativeAudioSource {
	...
	inline function streamValid():Bool {
		#if lime_vorbis
		return parent != null && parent.buffer != null && parent.buffer.__srcVorbisFile != null;
		#else
		return parent != null && parent.buffer != null;
		#end
	}

	private function streamTell():Float {
		#if lime_vorbis
		if (!streamValid()) return 0;
	...
	public function getCurrentTime():Float {
		...
		else if (parent.buffer == null) return length != null ? length : 0;
	...
	public function setCurrentTime(value:Float):Float {
		...
		if (streamed && !streamValid()) return value;
